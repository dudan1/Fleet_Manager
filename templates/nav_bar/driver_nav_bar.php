<div class ='nav_bar'>
    <img class = "logo_left" src="assets/images/fleet_manager_logo.PNG" alt="Fleet Manager" style="margin: auto;">

    <ul>
        <li><a href="driver_home.php">Home</a></li>
        <li><a href="driver_logs.php">My Logs</a></li>
        <li><a href="driver_view_profile.php">My Profile</a></li>
        <li><a href="driver_view_vehicle.php">My Vehicle</a></li>
        <li><a href="driver_contact.php">Contact Us</a></li>
        <li><a href="driver_help.php">Help</a></li>
        <li><a href="PHP_Scripts/logout.php">Log Out</a></li>
    </ul>
</div>